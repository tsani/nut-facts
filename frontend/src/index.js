...
const makeURL = (path, qs) =>
  path + '?' + new URLSearchParams(qs).toString();
...
function getFakeSearchResults (query) {
  ...
  return new Promise( (resolve, reject) =>
    resolve(fakeSearchData) )
    .then(data =>
      data.results.map(formatEdible).filter(matchesTerms));
}
...
function EdibleSelector(props) {
  ...
  useEffect(() => {
    if(props.searchTerms && null === props.selectedEdible) {
      getFakeSearchResults(props.searchTerms)
      .then(setEdibles)
    }
  ...
  useEffect(() => {
    if(null !== props.selectedEdible) {
      getFakeWeights(props.selectedEdible)
